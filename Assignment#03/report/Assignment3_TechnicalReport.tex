...
% \newunicodechar{|}{|} % or another suitable replacement
...
\subsection{Source and scale}\label{source-and-scale}
...
\item
  \textbf{Size:} 1,048,000 tweets ; CSV $\approx$ 219 MB ; XLSX $\approx$ 99 MB. We use
  the CSV throughout for $\sim$6$\times$ faster loading.
\item
...
\subsection{Label canonicalisation}\label{label-canonicalisation}

The raw \texttt{Category} column has \textbf{298 unique surface forms}
including \texttt{"\ Joy"},
...
After canonicalisation the \textbf{emotion task} uses 533,429 labelled
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{fig02_emotion_distribution.png}
\hfill
\includegraphics[width=0.48\textwidth]{fig03_sentiment_distribution.png}
\caption{Emotion and Sentiment Distributions}
...
\tightlist
\item
  \textbf{Emotion distribution (n = 533,429):} Joy 459,728 (86.2 \%);
  Sad 50,417 (9.5 \%); Disgust 17,083 (3.2 \%); Angry 2,802 (0.5 \%);
  Fear 1,847 (0.3 \%); Surprise 1,552 (0.3 \%).
\item
  \textbf{Derived sentiment distribution:} Positive 459,728 (86.2 \%),
  Negative 72,149 (13.5 \%), Neutral 1,552 (0.3 \%).
\item
  \textbf{Imbalance ratio (max / min)} $\approx$ 296$\times$ for the emotion task and $\approx$
  296$\times$ for the sentiment task. We address this with inverse-frequency
  class weights in every model's loss (Section 6).
\item
  \textbf{Token length:} Cleaned tweets have a median of \(\approx\) 11
  tokens and a 95th-percentile of \(\approx\) 36 tokens, which guides
  our \texttt{MAX\_LEN\ =\ 64} for the RNN/CNN models and
  \texttt{TRANSFORMER\_MAX\_LEN\ =\ 96} for subword tokenisation.
\end{itemize}

\subsection{Noise and code-mixing}\label{noise-and-code-mixing}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{fig13_label_leakage_heatmap.png}
\caption{Label Leakage Heatmap: Co-occurrence of specific emojis with emotion classes.}
\label{fig:label_leakage}
\end{figure}
...
\subsection{Suitability and limitations}\label{suitability-and-limitations}
...
\section{Related Work Summary (Milestone
...
Our work directly addresses all three: a million-scale corpus, identical
...
The pipeline is a single chain of modules with three swappable models
...
\subsection{Transformer encoders}\label{transformer-encoders}
...
\subsection{Data split and class
...
(sigmoid, 3-fold) \\
CNN & filters \(\{3,4,5\}\) $\times$ 128 ; dropout 0.5 ; Adam 1e-3 ; batch 256
; up to 8 epochs ; early-stop patience 2 \\
BiLSTM & hidden 256 (each dir) $\times$ 2 layers ; dropout 0.4 ; additive
...
\subsection{Hardware and runtime}\label{hardware-and-runtime}
...
\textasciitilde373 K training rows $\times$ 3 epochs $\times$ fp16 \\
...
\subsection{Leaderboards}\label{leaderboards}
...
\subsubsection{Sentiment (3-class)}\label{sentiment-3-class}
...
\subsubsection{Emotion (6-class)}\label{emotion-6-class}
...
a 1.7-point absolute gain over XLM-R (0.2535) and Urdu-RoBERTa (0.2539),
and a 2-point gain over Text-CNN (0.2499). The macro-F1 ceiling on the
6-class task is much lower than on the 3-class task because of the 296$\times$
class imbalance and the small absolute count of \emph{Surprise} ($\approx$1.5 K)
and \emph{Fear} ($\approx$1.8 K) training rows.
...
\texttt{outputs/figures/fig15\_cm\_emotion.png} (shown in Figure \ref{fig:confusion_matrices}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{fig15_cm_sentiment.png}
\hfill
\includegraphics[width=0.48\textwidth]{fig15_cm_emotion.png}
\caption{Row-normalised confusion matrices for sentiment and emotion tasks.}
...
\subsection{Macro vs weighted F1}\label{macro-vs-weighted-f1}
...
every model (see Figure \ref{fig:f1_compare}). 

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{fig16_f1_compare_sentiment.png}
\hfill
\includegraphics[width=0.48\textwidth]{fig16_f1_compare_emotion.png}
\caption{Macro vs Weighted F1 comparison across models.}
\label{fig:f1_compare}
\end{figure}

The Linear SVM bars show the largest gap ($\approx$0.44 on
sentiment, $\approx$0.63 on emotion), which is the canonical signature of a
...
\subsection{Training curves (CNN,
...
alongside each \texttt{.pt} checkpoint (see Figure \ref{fig:training_curves}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{fig17_curve_sentiment_bilstm.png}
\hfill
\includegraphics[width=0.48\textwidth]{fig17_curve_emotion_bilstm.png}
\caption{Training and validation curves for BiLSTM on sentiment and emotion tasks.}
\label{fig:training_curves}
\end{figure}

Both deep models converge within
...
\section{Discussion and Analysis}\label{discussion-and-analysis}

\subsection{Where models disagree}\label{where-models-disagree}
...
space has nearly zero discriminative signal for \emph{Surprise} ($\approx$1.5 K
rows) and \emph{Fear} ($\approx$1.8 K rows). Logistic Regression, fitted with
...
\subsection{Common error patterns}\label{common-error-patterns}
...
\tightlist
\item
  \textbf{Negation flips} - Urdu negators \emph{nahin}, \emph{mat},
  \emph{kabhi na} invert polarity but TF-IDF treats them as ordinary
  unigrams, so a sentence like \emph{``khushi nahin mili''} (no joy was
...
\subsection{Effect of label noise}\label{effect-of-label-noise}
...
\subsection{Classical vs Deep vs Transformer - measured
...
\begin{align*}
\text{Urdu-RoBERTa}\,(0.457) &\;>\; \text{Text-CNN}\,(0.453) \;\approx\; \text{mBERT}\,(0.453) \\
&\;>\; \text{BiLSTM-Attn}\,(0.450) \;>\; \text{XLM-R}\,(0.448) \\
&\;>\; \text{Linear SVM}\,(0.400) \;>\; \text{Logistic Reg.}\,(0.385).
\end{align*}

\textbf{Emotion (6-class) - macro-F1:}

\begin{align*}
\text{mBERT}\,(0.270) &\;>\; \text{Urdu-RoBERTa}\,(0.254) \;\approx\; \text{XLM-R}\,(0.254) \\
&\;>\; \text{Text-CNN}\,(0.250) \;>\; \text{BiLSTM-Attn}\,(0.243) \\
&\;>\; \text{Linear SVM}\,(0.209) \;>\; \text{Logistic Reg.}\,(0.163).
\end{align*}
...
\item
  ...
  noise. On emotion, mBERT's 0.020 lead over Text-CNN is real but small.
...
of 0.2703}. The CNN and BiLSTM baselines come within 0.004 / 0.028
...
\section{AI Usage Declaration}\label{ai-usage-declaration}
