...
The scope is limited to the local SentiUrdu-1M files and the two classification tasks. The final submission includes a lightweight Streamlit interface for leakage-aware preprocessing and local inference with the packaged Linear SVM model; saved outputs support inspection of the more expensive model families.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.7\columnwidth]{label_leakage_heatmap.png}
    \caption{Conditional association between sentiment derived from raw emojis and canonical categories. Strong cells indicate a shortcut risk if emojis remain in the input.}
    \label{fig:leakage}
\end{figure}
...
\subsection{Class-Weighted Objective}
...
\begin{equation}
\text{Loss} = -\frac{1}{B}\sum_{i=1}^{B}w_{y_i}\log p(y_i\mid x_i).
\end{equation}

\section{Implementation \& Codebase Architecture}

The project is structured as a modular Python package inside a self-contained folder. The layout is optimized for code cleanliness, automation, and grading visibility.

\subsection{Core Files and Preprocessing Configs}
\begin{itemize}[leftmargin=*]
    \item \texttt{config.yaml}: Central configuration file containing model hyperparameters, vocabulary sizes, sequence lengths, directory paths, and task-specific thresholds.
    \item \texttt{requirements.txt}: Pinned requirements list for maximum reproducibility (scikit-learn, PyTorch, transformers, streamlit, datasets).
\end{itemize}

\subsection{Implementation Modules in src/}
\begin{itemize}[leftmargin=*]
    \item \texttt{preprocessing.py}: Implementation of the 8-stage Urdu cleaning pipeline, including Unicode NFC normalization, URL stripping, hashtag preservation, and regex-based emoji extraction.
    \item \texttt{label\_mapping.py}: Automated mapping from noisy category inputs to canonical targets, resolving string whitespace, lists, and spelling errors.
    \item \texttt{create\_splits.py}: Splits the dataset into train, validation, and test sets.
    \item \texttt{train\_baseline.py}: Training logic for Logistic Regression, Multinomial Naive Bayes, and Linear SVM models.
    \item \texttt{train\_neural.py}: PyTorch loop for training Text-CNN and BiLSTM-Attention models, integrating early stopping and class-weighting.
    \item \texttt{train\_transformer.py}: Hugging Face Trainer integration for fine-tuning mBERT, XLM-R, and Urdu-RoBERTa models.
    \item \texttt{inference.py}: Clean, leak-free pipeline for processing custom inputs and predicting sentiment or emotion using baseline classifiers.
    \item \texttt{explanation\_assistant.py}: Prepares LIME-like explanations by calculating feature contributions to make the model predictions transparent.
\end{itemize}

\subsection{Runnable Scripts in scripts/}
Each numbered script acts as a stable CLI entry point for a specific step:
\begin{enumerate}
    \item \texttt{01\_prepare\_data.py}: Preprocesses raw data and splits it.
    \item \texttt{02\_train\_classical.py}: Fits TF-IDF and traditional classifiers.
    \item \texttt{03\_train\_neural.py}: Compiles neural classifiers.
    \item \texttt{04\_train\_transformers.py}: Fine-tunes transformers.
    \item \texttt{05\_evaluate\_models.py}: Evaluates checkpoints and generates metrics.
    \item \texttt{06\_generate\_visualizations.py}: Compiles and exports Matplotlib charts.
\end{enumerate}

\subsection{Testing \& Validation Modules}
\begin{itemize}[leftmargin=*]
    \item \texttt{tests/}: 45+ automated regression and verification checks.
    \item \texttt{validate\_final\_project.py}: Project-wide health checks ensuring all datasets, splits, check-pointing folders, predictions, notebook reports, and figures exist.
\end{itemize}
...
Figure~\ref{fig:distribution} and Tables~\ref{tab:emotion-dist} \& \ref{tab:sentiment-dist} show the canonical emotion and sentiment distributions before empty-text removal. Joy accounts for 86.2\% of labeled rows; Surprise and Fear each contribute less than 0.4\%. The imbalance ratio between Joy and Surprise is approximately 296:1. The derived sentiment distribution is similarly skewed.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\columnwidth]{emotion_distribution.png}
...
\begin{figure}[t]
    ...
    \caption{Preprocessed class frequency distribution highlighting massive imbalance.}
    \label{fig:class-dist-plot}
\end{figure}
...
Table~\ref{tab:orig-emotion-results} shows a more difficult six-class problem. Linear SVM again reaches the highest accuracy (0.8773), yet its macro-F1 falls to 0.2087 because it predicts Joy for nearly all minority examples. mBERT achieves the strongest macro-F1 at 0.2703, ahead of Urdu-RoBERTa (0.2539), XLM-R (0.2535), and Text-CNN (0.2499). The best transformer advantage over Text-CNN is approximately 0.0204 macro-F1: meaningful in the observed leaderboard, but still modest relative to the remaining error.

The mBERT confusion matrix reveals the minority-class bottleneck. Joy recall is 0.73 and Sad recall is 0.58, but Angry recall is 0.17, Fear 0.10, Disgust 0.31, and Surprise 0.05. Half or more of Angry, Fear, and Surprise examples are predicted Joy. Class weighting encourages minority predictions, but it cannot create reliable linguistic evidence when labels are scarce and noisy.

\subsection{Packaged Rerun Results}
A subsequent packaged rerun (documented in Table~\ref{tab:packaged-rerun-results}) was performed on a 517,966-row subset using a 2-token minimum filter. 

In this setup, **Linear SVM** achieves the highest Macro-F1 of **0.5040** (accuracy 0.8531), outperforming the deep sequence and transformer models (which used smaller training resource budgets in this subset run). This demonstrates that classical TF-IDF models remain highly competitive on Urdu Twitter text when sufficient vocabulary size (50,000) and unigram/bigram features are preserved.

\subsection{Diagnostic and Performance Visualizations}
The retained visual diagnostics in Figures~\ref{fig:baseline-performance}--\ref{fig:f1-comparison-charts} focus on the paper's central evidence: baseline metric trade-offs, transformer comparisons, and the gap between macro-F1 and weighted-F1. Lower-value diagnostic grids were removed from the camera-ready version because they were visually dense in IEEE two-column format and did not add as much evidence as the tables and discussion.

\begin{figure}[t]
    \centering
    \subfigure[Baseline F1 Comparisons]{\includegraphics[width=0.48\columnwidth]{baseline_model_macro_f1_comparison.png}}
    \subfigure[Baseline Accuracy vs F1]{\includegraphics[width=0.48\columnwidth]{baseline_model_accuracy_vs_macro_f1.png}}
    \caption{Baseline model metrics highlighting the tradeoff between overall accuracy and minority class F1 scores.}
    \label{fig:baseline-performance}
\end{figure}

\begin{figure}[t]
    \centering
    \subfigure[Transformer vs Baseline/Neural]{\includegraphics[width=0.48\columnwidth]{transformer_vs_baseline_neural_macro_f1.png}}
    \subfigure[Transformer Macro-F1]{\includegraphics[width=0.48\columnwidth]{transformer_model_macro_f1_comparison.png}}
    \caption{Fine-tuned Transformer models compared to deep sequence and traditional classifiers.}
    \label{fig:transformer-performance}
\end{figure}

\begin{figure}[t]
    \centering
    \subfigure[Sentiment F1 Comparison]{\includegraphics[width=0.48\columnwidth]{sentiment_f1_comparison.png}}
    \subfigure[Emotion F1 Comparison]{\includegraphics[width=0.48\columnwidth]{emotion_f1_comparison.png}}
    \caption{Gaps between Macro-F1 and Weighted-F1 across sentiment and emotion tasks, showing majority-class bias.}
    \label{fig:f1-comparison-charts}
\end{figure}

\subsection{Macro Versus Weighted Performance}
Figure~\ref{fig:f1-comparison-charts} makes the imbalance effect explicit. Linear SVM has a sentiment weighted-F1 of 0.8409 but macro-F1 of 0.4004; on emotion the gap expands from 0.8347 weighted-F1 to 0.2087 macro-F1. A deployment judged only by accuracy or weighted-F1 could therefore appear strong while failing most rare emotional states.
